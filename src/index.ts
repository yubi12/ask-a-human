...
mcpServer.registerTool(
  ...
  {
    title: "Ask a Human",
    description: `Pause execution and ask a human a question via ${platformName === "discord" ? "Discord" : "Slack"}. The human replies in a thread and execution resumes with their answer. Pass the returned thread_id in subsequent calls to continue the conversation in the same thread.`,
    inputSchema: {
      ...
      context: z
        ...
        .optional()
        .describe(
          "Background context to help the human understand the question",
        ),
      options: z
        .array(z.string())
        .optional()
      ...
      thread_id: z
        ...
        .optional()
        .describe(
          "Thread ID from a previous ask_human response to continue the conversation in the same thread",
        ),
    },
    annotations: {
      readOnlyHint: false,
  ...
  async ({ question, context, options, thread_id }, extra) => {
    const key = await platform.postQuestion({
      question,
      context,
      options,
      thread_id,
    ...
    // Optional timeout
    let timeoutPromise: Promise<QuestionResult> | undefined;
    if (ASK_TIMEOUT_MS > 0) {
      timeoutPromise = new Promise<QuestionResult>((resolve) => {
        pending.timeoutId = setTimeout(
          () => resolve(SENTINEL_TIMEOUT),
          ASK_TIMEOUT_MS,
        );
      });
    }
    ...
    const reply: QuestionResult = timeoutPromise
